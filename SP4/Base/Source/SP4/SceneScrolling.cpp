...
void SceneScrolling::Init()
{
	...
	bossDoorEntity = Create::Entity(entityManager, "bossdoor", 
		Vector3(halfWorldWidth,worldHeight + halfWorldWidth , -2),
		Vector3(worldWidth, worldWidth, 0)
		);
	yDestination = groundEntity->GetPosition().y + groundEntity->GetScale().y * 0.5f;
	bossDoorEntity->SetShouldRender(false);
...
void SceneScrolling::WavesInit()
{
    ...
    { // Initializing Waves
        ...
        wave1.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 1.0f, 2.0f, 1.0f, Vector3(20, 110), Vector3(3, 3, 1)));
        wave1.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 1.0f, 2.0f, 1.0f, Vector3(80, 110), Vector3(3, 3, 1)));
        wave1[0]->AddWaypoint(Vector3(40, 45, 0));
        wave1[1]->AddWaypoint(Vector3(60, 45, 0));

        std::vector<EnemyBase*> wave2;
        wave2.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 1.0f, 2.0f, 1.0f, Vector3(-20, 80), Vector3(3, 3, 1)));
        wave2.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 1.0f, 2.0f, 1.0f, Vector3(150, 80), Vector3(3, 3, 1)));
        wave2[0]->AddWaypoint(Vector3(20, 45, 0));
        wave2[0]->AddWaypoint(Vector3(80, 45, 0));
        wave2[1]->AddWaypoint(Vector3(80, 40, 0));
        wave2[1]->AddWaypoint(Vector3(20, 40, 0));

        std::vector<EnemyBase*> wave3;
        Bitey* bitey = Create::biteyAsset("bitey", 200.0f, 20.0f, 0.0f, 2.0f, 2.0f, Vector3(20, 110), Vector3(3, 3, 1));
        bitey->SetBoundary(Vector3(0, 0, 0), Vector3(worldWidth, camera->GetPosition().y + orthoHeight * 0.5f, 0));
        wave3.push_back(bitey);
        bitey = Create::biteyAsset("bitey", 200.0f, 20.0f, 0.0f, 2.0f, 2.0f, Vector3(50, 110), Vector3(3, 3, 1));
        bitey->SetBoundary(Vector3(0, 0, 0), Vector3(worldWidth, camera->GetPosition().y + orthoHeight * 0.5f, 0));
        wave3.push_back(bitey);
        bitey = Create::biteyAsset("bitey", 200.0f, 20.0f, 0.0f, 2.0f, 2.0f, Vector3(80, 110), Vector3(3, 3, 1));
        bitey->SetBoundary(Vector3(0, 0, 0), Vector3(worldWidth, camera->GetPosition().y + orthoHeight * 0.5f, 0));
        wave3.push_back(bitey);
        wave3[0]->AddWaypoint(Vector3(45, 40, 0));
        wave3[1]->AddWaypoint(Vector3(50, 40, 0));
        wave3[2]->AddWaypoint(Vector3(55, 40, 0));

        std::vector<EnemyBase*> wave4;
        bitey = Create::biteyAsset("bitey", 200.0f, 20.0f, 0.0f, 2.0f, 2.0f, Vector3(20, 110), Vector3(3, 3, 1));
        bitey->SetBoundary(Vector3(0, 0, 0), Vector3(worldWidth, camera->GetPosition().y + orthoHeight * 0.5f, 0));
        wave4.push_back(bitey);
        bitey = Create::biteyAsset("bitey", 200.0f, 20.0f, 0.0f, 2.0f, 2.0f, Vector3(80, 110), Vector3(3, 3, 1));
        bitey->SetBoundary(Vector3(0, 0, 0), Vector3(worldWidth, camera->GetPosition().y + orthoHeight * 0.5f, 0));
        wave4.push_back(bitey);
        wave4[0]->AddWaypoint(Vector3(30, 47, 0));
        wave4[0]->AddWaypoint(Vector3(55, 47, 0));
        wave4[1]->AddWaypoint(Vector3(80, 47, 0));
        wave4[1]->AddWaypoint(Vector3(45, 47, 0));
        ...
        {//Bities
            ...
            bitey->SetBoundary(Vector3(0, 0, 0), Vector3(worldWidth, camera->GetPosition().y + orthoHeight * 0.5f, 0));
            wave5.push_back(bitey);
            wave5[0]->AddWaypoint(Vector3(15, 53, 0));
            wave5[0]->AddWaypoint(Vector3(45, 53, 0));
            wave5[1]->AddWaypoint(Vector3(25, 53, 0));
            wave5[1]->AddWaypoint(Vector3(55, 53, 0));
            wave5[2]->AddWaypoint(Vector3(35, 50, 0));
            wave5[2]->AddWaypoint(Vector3(65, 50, 0));
            wave5[3]->AddWaypoint(Vector3(45, 50, 0));
            wave5[3]->AddWaypoint(Vector3(75, 50, 0));
        }
        {
            wave5.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 0.0f, 2.0f, 1.0f, Vector3(30, 110), Vector3(3, 3, 1)));
            wave5.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 0.0f, 2.0f, 1.0f, Vector3(40, 110), Vector3(3, 3, 1)));
            wave5.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 0.0f, 2.0f, 1.0f, Vector3(50, 110), Vector3(3, 3, 1)));
            wave5.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 0.0f, 2.0f, 1.0f, Vector3(60, 110), Vector3(3, 3, 1)));
            wave5.push_back(Create::floopyAsset("floopy", 200.0f, 20.0f, 0.0f, 2.0f, 1.0f, Vector3(70, 110), Vector3(3, 3, 1)));
...
void SceneScrolling::Update(double dt)
{
	...
	if (!waveCleared)//if wave not cleared
	{
		...
		if (waveList.size() > 0)//safety measure in case of an empty vector
		{
			...
			if (waveList[waveIndex].size() <= 0)
			{
				...
				{
					waveCleared = true;
					bossDoorEntity->SetShouldRender(true);
					cloudFactory->SetIsDone(true);
				}
	...
	else//wave cleared
	{
		if (cutSceneFinished)
		{
			{//Unlocks camera
				float avrgYPos = 0;
				if (GameInfo::GetInstance()->GetPlayersList().size() > 0)
				{
					for (auto q : GameInfo::GetInstance()->GetPlayersList())
						avrgYPos += q->GetCharacter()->GetPosition().y;
					avrgYPos /= GameInfo::GetInstance()->GetPlayersList().size();
				}
				float yUpperLimit = bossDoorEntity->GetPosition().y + bossDoorEntity->GetScale().y * 0.5f;
				if (avrgYPos + (orthoHeight * 0.5f) > yUpperLimit)
				{
					avrgYPos = yUpperLimit - orthoHeight * 0.5f;
				}
				float value = Math::Lerp(camera->GetPosition().y, avrgYPos, 2.0f * dt);
				camera->SetPosition(Vector3(worldWidth * 0.5f, value, 1));
				camera->SetTarget(Vector3(worldWidth * 0.5f, value, 0));
			}
			{//Checks if players reached bossDoor
				short counter = 0;
				for (auto q : GameInfo::GetInstance()->GetPlayersList())
				{
					Vector3 playerPos = q->GetCharacter()->GetPosition() + q->GetCharacter()->GetColliderOffset();
					
					if (playerPos.x < bossDoorEntrance->GetPosition().x + bossDoorEntrance->GetScale().x * 0.5f &&
						playerPos.x > bossDoorEntrance->GetPosition().x - bossDoorEntrance->GetScale().x * 0.5f &&
						playerPos.y < bossDoorEntrance->GetPosition().y + bossDoorEntrance->GetScale().y * 0.5f &&
						playerPos.y > bossDoorEntrance->GetPosition().y - bossDoorEntrance->GetScale().y * 0.5f)
					{
						counter++;
					}
				...
				if (counter == GameInfo::GetInstance()->GetPlayersList().size())
				{
					SceneManager::GetInstance()->SetActiveScene("SceneBoss");
					return;
				}
			}
		}
		...
		{
			{//Move in boss door
				Vector3 pos = bossDoorEntity->GetPosition();
				pos.y -= scrollSpeed * dt;
				if (pos.y < yDestination)
					pos.y = yDestination;
				if (pos.y == yDestination && flapflapXoffset == 0.0f)
				{
					cutSceneFinished = true;
					{//for bossdoor
						floorList.push_back(Create::floorTileEntity(entityManager, "quad2", Vector3(worldWidth * 0.5f, 60.0f, -0.9f), Vector3(30.0f, 50.0f, 0)));
						bossDoorEntrance = Create::floorTileEntity(entityManager, "quad2", Vector3(worldWidth * 0.5f, 78.5f, -0.9f), Vector3(32.0f, 5.0f, 0));
						floorList.back()->SetShouldRender(floorList[0]->GetShouldRender());
						bossDoorEntrance->SetShouldRender(floorList[0]->GetShouldRender());
						floorList.push_back(bossDoorEntrance);

						Wall* wall = Create::squareWall(entityManager, "quad2", Vector3(worldWidth * 0.5f, 83.0f, -0.9f), Vector3(32.0f, 10.0f, 0));
						//Wall* wall = Create::wall(entityManager, "quad2", Vector3(worldWidth * 0.5f, 55.5f, -0.9f), Vector3(10, 10, 0));
						wall->SetShouldRender(false);

						wall = Create::squareWall(entityManager, "quad2", Vector3(worldWidth * 0.5f + 17.5f, 60.0f, -0.9f), Vector3(5.0f, 50.0f, 0));
						wall->SetShouldRender(false);
						wall = Create::squareWall(entityManager, "quad2", Vector3(worldWidth * 0.5f - 17.5f, 60.0f, -0.9f), Vector3(5.0f, 50.0f, 0));
						wall->SetShouldRender(false);
					}
				}
				bossDoorEntity->SetPosition(pos);
			}
			{//Move out clouds
				if (cloudScrollUI[0]->GetScale().x < canvasWidth * 4.0f)
				{
					Vector3 newScale = cloudScrollUI[0]->GetScale();
					newScale.x += 10.0f * dt;
					if (newScale.x >= canvasWidth * 4.0f)
					{
						newScale.x = canvasWidth * 4.0f;
					}

					cloudScrollUI[0]->SetScale(newScale);
					cloudScrollUI[1]->SetScale(newScale);
				}
			}
			{//Center manta
				flapflapDirection = -Math::GetSign(flapflapXoffset);
				flapflapXoffset += flapflapOffsetSpeed * flapflapDirection * dt;
				if (flapflapXoffset * flapflapDirection > 0.0f)
				{
					flapflapDirection = 0;
					flapflapXoffset = 0;
				}
