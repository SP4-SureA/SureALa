								...
								const float yOffset)
{
		...
		&& (xNumOfGrid>0) && (zNumOfGrid>0))
	{
		...
		theGrid = new Grid[ xNumOfGrid*zNumOfGrid ];
...
void CSpatialPartition::Render(Vector3* theCameraPosition)
{
	...
	for (int i = 0; i<xNumOfGrid; i++)
	{
		for (int j = 0; j<zNumOfGrid; j++)
		{
			...
			modelStack.Translate(0.5f, 0.0, 0.5f);
...
Get yOffset
********************************************************************************/
float CSpatialPartition::GetyOffset(void) const
{
	return yOffset;
}
/********************************************************************************
 ...
 ********************************************************************************/
Grid CSpatialPartition::GetGrid(const int xIndex, const int zIndex) const
{
...
Get a particular grid
********************************************************************************/
Grid CSpatialPartition::GetGrid(Vector3 position) const
{
...
/********************************************************************************
Sets camera for spatial partition
********************************************************************************/
...
/********************************************************************************
Removes spatial partition camera
********************************************************************************/
...
********************************************************************************/
bool CSpatialPartition::IsVisible(Vector3 cameraPos,
	Vector3 cameraDir,
	const int xIndex, const int zIndex)
{
	float xDistance = (xGridSize*xIndex + (xGridSize >> 1) - (xSize >> 1)) - cameraPos.x;
	float zDistance = (zGridSize*zIndex + (zGridSize >> 1) - (zSize >> 1)) - cameraPos.z;
	...
	if (cameraDir.Dot(gridCentre) < 0)
		return false;
	else
		return true;
}

bool CSpatialPartition::InRenderRange(float distance)
{
	if (distance < renderRange * renderRange)
		return true;
